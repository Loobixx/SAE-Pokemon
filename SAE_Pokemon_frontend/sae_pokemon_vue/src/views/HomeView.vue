  ...
  // --- CHANGEMENT 1 : CLÉS UNIQUES (ID + SHINY) ---
  // Au lieu de regarder juste le numéro, on crée une signature unique "NUMERO-SHINY"
  // Exemple : "1-false" (Bulbizarre Normal) vs "1-true" (Bulbizarre Shiny)
  ...
  const baseFilteredPokemons = computed(() => {
    ...
    // 2. ONGLET SOUHAITÉS
      ...
      return wishedList.value
        .filter(wish => {
          // Filtre : on cache si déjà attrapé dans la MÊME version
    ...
    if (currentTab.value === 'pokedex') {
      return pokemons.value.filter(p => p.numero <= settings.maxId);
    }
  ...
  const fetchCapturedList = async () => {
    ...
    try {
      ...
      if (response.ok) {
        capturedList.value = await response.json();
      }
  ...
  const fetchWishedList = async () => {
    ...
    try {
      ...
      if (response.ok) {
        wishedList.value = await response.json();
      }
    } catch (e) { console.error(e); }
  };
        ...
        <v-container fluid class="py-8">
          ...
          <v-row justify="center" class="mb-6">
            <v-col cols="12" class="text-center">
              <h1 class="text-h3 font-weight-bold mb-2 text-primary-dark">
                {{ pageTitle }}
              </h1>

              <p class="text-subtitle-1 text-grey-darken-1">
                {{ baseFilteredPokemons.length }} Pokémon(s)
                <span v-if="currentTab === 'pokedex'" class="text-caption">
                  (Filtre: Max {{ settings.maxId }})
                </span>
              </p>
          ...
          <v-row v-else-if="error" justify="center" class="mt-5">
            <v-col cols="12" md="8">
              <v-alert type="error" variant="tonal" title="Erreur de chargement">
                {{ error }}
              </v-alert>
            </v-col>
          </v-row>
          ...
          <v-row v-else-if="baseFilteredPokemons.length === 0" justify="center" class="mt-10">
            <v-col cols="12" class="text-center empty-state">
              ...
              <h3 class="text-h5 text-grey mt-4">
                {{ currentTab === 'wish' ? 'Tous vos vœux sont exaucés !' : 'Aucun Pokémon ici.' }}
              </h3>
              <p class="text-body-1 text-grey-darken-1">
                {{ currentTab === 'wish' ? 'Vous avez tout attrapé.' : 'Allez explorer le Pokédex.' }}
              </p>
            </v-col>
          </v-row>
